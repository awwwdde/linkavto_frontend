...
function LoginPill() {
  ...
  if (user) {
    ...
    const letters = userInitials(user.first_name, user.last_name, user.email)
    // Ширина фиксирована: имя переменной длины раньше двигало всю правую часть
    // хэдера, инициалы этого не делают.
    return (
      <Link
        to="/profile"
        title={fullName}
