    ...
    <AnimatePresence initial={false}>
      {open ? (
        <div className="fixed inset-0 z-50 flex items-end justify-center p-0 sm:items-center sm:p-6">
          ...
          >
            {/* Со скрытым заголовком строка шапки не нужна: крестик уходит в
                угол абсолютом, иначе он резервировал бы 40 px пустоты и
                содержимое начиналось бы заметно ниже верхнего края. */}
            <div className={cn('flex items-start justify-between gap-4', !hideTitle && 'mb-4')}>
              ...
              <button
                ...
                className={cn(
                  'flex h-10 w-10 items-center justify-center rounded-control text-ink-muted hover:bg-ink/5',
                  hideTitle ? 'absolute top-4 right-4' : '-mr-2 -mt-2 ml-auto',
                )}
              >
                <IconClose />
              </button>
          ...
          >
            <div className="flex items-center justify-between gap-4 border-b border-line px-4 py-4">
              ...
              <button
                ...
                onClick={onClose}
                aria-label={t('nav.close')}
                className="ml-auto flex h-10 w-10 items-center justify-center rounded-control text-ink-muted hover:bg-ink/5"
              >
                <IconClose />
              </button>
            </div>
