...
/* --- Покупатели (мок): аккаунт создаётся при первом входе по коду и живёт в
   localStorage. Держать их в памяти модуля нельзя — service worker переживает
   перезагрузку не всегда, и пользователь терял бы профиль вместе со страницей. --- */
const USERS_KEY = 'linkavto:mock-users'
...
function saveUsers(map: Record<string, AuthUser>): void {
  try {
    localStorage.setItem(USERS_KEY, JSON.stringify(map))
  } catch {
    /* приватный режим — аккаунт живёт только в этой сессии */
  }
}
...
function putUser(user: AuthUser): void {
  const users = loadUsers()
...
/*
 * Токен мока — это сам адрес почты в base64. Реальный бэкенд разбирает JWT и
 * достаёт оттуда пользователя; здесь тот же принцип, только без подписи: кто
 * пришёл, видно из запроса, а не из переменной, которая обнуляется перезагрузкой.
 */
const TOKEN_PREFIX = 'mock.'

const issueToken = (email: string): string => TOKEN_PREFIX + btoa(encodeURIComponent(email))
...
function sessionEmail(request: Request): string | null {
  ...
  try {
    return decodeURIComponent(atob(token.slice(TOKEN_PREFIX.length)))
  } catch {
    return null
  }
}
...
export const handlers = [
  ...
  http.post(`${BASE}/auth/email-code/verify/`, async ({ request }) => {
    ...
    return HttpResponse.json({ token: issueToken(email), user: getOrCreateUser(email) })
  }),

  /* --- Профиль покупателя --- */

  http.get(`${BASE}/account/`, async ({ request }) => {
    await delay(120)
    const email = sessionEmail(request)
    if (!email) return new HttpResponse(null, { status: 401 })
    return HttpResponse.json(getOrCreateUser(email))
  }),
  ...
  http.patch(`${BASE}/account/`, async ({ request }) => {
    ...
    const user = { ...getOrCreateUser(email), ...patch, email }
  ...
  http.post(`${BASE}/orders/`, async ({ request }) => {
    ...
    const email = sessionEmail(request) ?? 'guest'
  ...
  http.get(`${BASE}/orders/`, async ({ request }) => {
    await delay(200)
    const list = loadOrders()[sessionEmail(request) ?? 'guest'] ?? []
    return HttpResponse.json({ count: list.length, next: null, previous: null, results: list })
  }),

  http.get(`${BASE}/orders/:id/`, async ({ params, request }) => {
    await delay(150)
    const list = loadOrders()[sessionEmail(request) ?? 'guest'] ?? []
