...
export function AuthModal() {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      // Имя, фамилия и телефон заполняются в личном кабинете, отдельного шага
      // после входа больше нет — пользователь сразу возвращается на страницу.
      finish()
    } catch (error) {
  ...
  // На шаге кода приглушённой строкой показываем, куда он ушёл; у шага входа
  // подзаголовка нет — остаётся только «Вход.».
  const headLead = step === 'email' ? t('auth.heading') : t('auth.codeHeading')
  const headGhost = step === 'code' ? `${t('auth.codeHint')} ${email}` : undefined

  return (
    <Modal open={open} onClose={close} title={t('auth.title')} hideTitle>
      {/* pr-12 — чтобы длинный заголовок шага с кодом не уезжал под крестик. */}
